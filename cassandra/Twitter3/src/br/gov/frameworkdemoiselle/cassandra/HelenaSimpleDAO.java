...
public abstract class HelenaSimpleDAO<T> extends AbstractHelenaDAO<T> {
        ...
        for ( final PropertyDescriptor d : propertyDescriptors ) {
            if ( isReadWrite( d ) ) {
                try {
                    ...
                    final byte[] value = typeConverter.convertValueObjectToByteArray( PropertyUtils.getProperty( object, name ) );
    ...
    private T applyColumns( final String key, final Iterable<Column> slice ) {
        try {
            ...
            PropertyUtils.setProperty( newInstance, keyPropertyDescriptor.getName(),
                    typeConverter.convertByteArrayToValueObject( keyPropertyDescriptor.getReadMethod().getReturnType(),
            ...
            for ( final Column c : slice ) {
                ...
                if ( PropertyUtils.isWriteable( newInstance, name ) ) {
                    ...
                    PropertyUtils.setProperty( newInstance, name, typeConverter.convertByteArrayToValueObject( returnType, c.value ) );
                }
            }
            ...
            throw new HelenaRuntimeException("Could not instanciate " + clz.getName(), e );
        } catch ( final IllegalAccessException e ) {
            throw new HelenaRuntimeException("Could not instanciate " + clz.getName(), e );
        } catch ( final InvocationTargetException e ) {
            throw new HelenaRuntimeException( e );
    ...
    private SlicePredicate makeSlicePredicateWithColumns( final Iterable<String> columns ) {
        final SlicePredicate predicate = new SlicePredicate();
        predicate.setColumn_names( ImmutableList.copyOf( Iterables.transform( columns, typeConverter.toByteArrayFunction() ) ) );
        return predicate;
    }
